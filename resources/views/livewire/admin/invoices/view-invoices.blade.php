    ...
    <div class="row mb-2 mb-xl-3">
        ...
        @if (Auth::user()->can('add_invoice'))
            <div class="col-auto ms-auto text-end mt-n1">
                <a href="{{ route('admin.add_invoice') }}"
                    class="btn btn-primary">{{ $lang->data['new_invoice'] ?? 'New Invoice' }}</a>
            </div>
        @endif
    </div>
        ...
        <div class="col-12">
            <div class="card p-0">
                <div class="card-body p-0">
                                    ...
                                    <td>
                                        @if (Auth::user()->can('invoice_list'))
                                            @if ($item->first_invoice == 100)
                                                <a href="{{ url('/admin/invoices/print/' . $item->id . '/1' . '/' . $item->first_invoice . '/0') }}"
                                                    class="btn btn-sm btn-success">{{ $lang->data['print_invoice'] ?? 'Print invoice' }}</a>
                                                @if ($item->first_invoice_amount == $item->first_invoice_paid)
                                                    <a href="#"
                                                        class="btn btn-sm btn-success">{{ $lang->data['pay_invoice'] ?? 'Invoice Paid' }}</a>
                                                @else
                                                    <a href="#" class="btn btn-sm btn-danger"
                                                        data-bs-toggle="modal" data-bs-target="#Modalpayment"
                                                        wire:click='payment({{ $item }},0)'>{{ $lang->data['pay_invoice'] ?? 'Pay invoice' }}<br>{{ $lang->data['first_due_date'] ?? 'Due Date:' . $item->first_due_date }}</a>
                                                @endif
                                            @endif

                                            @if ($item->first_invoice < 100)
                                                <a href="{{ url('/admin/invoices/print/' . $item->id . '/1' . '/' . $item->first_invoice . '/1') }}"
                                                    class="btn btn-sm btn-success">{{ $lang->data['print_invoice'] ?? 'Print invoice' . $item->invoice_number . '-1' }}</a>
                                                @if ($item->first_invoice_amount == $item->first_invoice_paid)
                                                    <a href="#"
                                                        class="btn btn-sm btn-success">{{ $lang->data['pay_invoice'] ?? 'First Invoice Paid' }}</a>
                                                @else
                                                    <a href="#" class="btn btn-sm btn-danger"
                                                        data-bs-toggle="modal" data-bs-target="#Modalpayment"
                                                        wire:click='payment({{ $item }},1)'>{{ $lang->data['pay_invoice'] ?? 'Pay invoice' . $item->invoice_number . '-1' }}<br>{{ $lang->data['first_due_date'] ?? 'Due Date:' . $item->first_due_date }}</a>
                                                @endif
                                            @endif
                                        @endif
                                    </td>
                                    <td>
                                        @if (Auth::user()->can('invoice_list'))
                                            @if ($item->first_invoice < 100)
                                                ...
                                                @endphp
                                                <a href="{{ url('/admin/invoices/print/' . $item->id . '/1' . '/' . $per . '/2') }}"
                                                    class="btn btn-sm btn-success">{{ $lang->data['print_invoice'] ?? 'Print invoice' . $item->invoice_number . '-2' }}</a>
                                                @if ($item->second_invoice_amount == $item->second_invoice_paid)
                                                    <a href="#"
                                                        class="btn btn-sm btn-success">{{ $lang->data['pay_invoice'] ?? 'Second Invoice Paid' }}</a>
                                                @else
                                                    <a href="#" class="btn btn-sm btn-danger"
                                                        data-bs-toggle="modal" data-bs-target="#Modalpayment"
                                                        wire:click='payment({{ $item }},2)'>{{ $lang->data['pay_invoice'] ?? 'Pay invoice' . $item->invoice_number . '-2' }}<br>{{ $lang->data['second_due_date'] ?? 'Due Date:' . $item->second_due_date }}</a>
                                                @endif
                                            @endif
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        @if (Auth::user()->can('invoice_list'))
                                            @if (Auth::user()->can('edit_invoice'))
                                                <a href="{{ route('admin.edit_invoice', $item->id) }}"
                    ...
                    @if (count($invoices) == 0)
                        <x-no-data-component
                            message="{{ $lang->data['no_invoices_found'] ?? 'No invoices were found..' }}" />
                    @endif
                </div>
            </div>
        </div>
